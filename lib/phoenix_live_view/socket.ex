defmodule Phoenix.LiveView.Socket do
  ...
  def connect(_params, %Phoenix.Socket{} = socket, connect_info) do
	...
	case Guardian.Phoenix.Socket.authenticate(socket, Elenchos.Authentication.Guardian, session["guardian_default_token"]) do
	  {:ok, authed_socket} ->
	    IO.inspect(put_in(authed_socket.private[:session], session))
	    {:ok, put_in(authed_socket.private[:session], session)}
